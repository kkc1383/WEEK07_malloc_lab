...
#define ALIGN(size) (((size) + WSIZE + (ALIGNMENT - 1)) & ~0x7) //8byte 기준으로 정렬, 올림, (ALIGNMENT-1)을 더해주고 나누어서 0이 아니라면 무조건 몫이 1이 오름 bias 개념
...
#define PREV_FL(bp) (*(void **)(bp))
#define NEXT_FL(bp) (*(void **)((char *)(bp) + DSIZE))   
#define GET_PREV_FREE(p) (GET(p) & (1<<1)) // p라는 포인터가 가리키는 헤더의 prev_free 유무 (뒤에서 두번째 비트 1자리), 이건 쓰일지 잘 모르겠음
...
int mm_init(void)
{
    ...
    PUT(heap_listp + WSIZE, PACK(0,1)); // 그리고 프롤로그헤더 다음 바로 에필로그 와도 됨
...
void *mm_malloc(size_t size)
{
    ...
    int asize = ALIGN(size); // no footer라서 헤더값만 더했음
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    size_t newsize    = ALIGN(size);           // 새 사이즈
...
static void* extend_heap(size_t words){
    ...
    //에필로그 블록은 prev_free 필요없어서 별도 세팅 안함
...
static void place(void* bp, size_t asize){ // 이 함수는 블록을 받아서 값 입력을 했는데, 남는 공간이 있을 수도 있으니까 그거 분할하는 과정도 포함되어 있음.
    ...
    if((csize- asize)>=(3*DSIZE)){ // 남는 크기가 2word 이상이면 남는 부분 free하기
        ...
        SET_PREV_FREE(HDRP(bp),0); // 분할된 블록 헤더의 prev_free 비트 세팅
        SET_PREV_FREE(FTRP(bp),0); // 분할된 블록 풋터의 prev_free 비트 세팅
        void* newptr=coalesce(bp); // 이걸 안했나?
        addFreeBlock(newptr);
    }
...
static void addFreeBlock(void* newPtr){
    if(fl_head==NULL){ // free list에 아무것도 없다면
        PREV_FL(newPtr)=NULL; // 시작 노드의 이전도 없고
        NEXT_FL(newPtr)=NULL; // 다음도 없음
        fl_head=newPtr; // newptr을 헤드로 임명
    }
    else{ // free list에 하나라도 노드가 있다면, LIFO니까 맨 앞에 추가
        PREV_FL(newPtr)=NULL; // 맨 앞에 올 노드니까 이전은 없고
        NEXT_FL(newPtr)=fl_head; // 다음 노드가 원래 헤드였던 노드
        PREV_FL(fl_head)=newPtr; // 그리고 원래 헤드였던 노드의 이전이 추가한 블락이고
...
static void deleteFreeBlock(void* delPtr){
    ...
    if(fl_head==delPtr){ // 헤드 노드를 삭제 할 경우
        fl_head=NEXT_FL(delPtr);// 헤드 다음의 노드가 헤드가 될테니
        if(!fl_head) return; // 리스트가 하나밖에 없는 경우
        else PREV_FL(fl_head)=NULL; // 새로운 헤드의 이전은 없어야 하니 널로 설정
    }
    else{
        NEXT_FL(PREV_FL(delPtr))=NEXT_FL(delPtr); // 이전노드의 다음이 삭제할 노드의 다음
        PREV_FL(NEXT_FL(delPtr))=PREV_FL(delPtr); // 다음 노드의 이전이 삭제할 노드의 이전
    }
}