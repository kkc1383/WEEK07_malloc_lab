...
#define GET_PREV_FREE(p) ((GET(p)>>1) & 0x1) // p라는 포인터가 가리키는 헤더의 prev_free 유무 (뒤에서 두번째 비트 1자리), 이건 쓰일지 잘 모르겠음
#define SET_PREV_FREE(p,bit) (*((unsigned int *)(p)) = (GET(p)&(~0x2))|(bit<<1)) // bit 값을 받아서 헤더의 prev_free 자리에 비트 세팅. 하는 방법은 먼저 뒤에서 두번째 비트를 0으로 초기화 시켜준 후, 그 자리에 요청한 bit 값을 넣으면 됨. 기존의 비트들을 건드리면 안되서 이렇게 해야함
...
static void* fl_head=NULL; // free list 링크드 리스트의 헤드
static void addFreeBlock(void* newPtr);
static void deleteFreeBlock(void* delPtr);
...
void mm_free(void *ptr)
{
    ...
    PUT(FTRP(ptr), PACK(size,0));// 풋터에 free임을 알리기위해서 alloc_bit를 0으로 새로 갱신 , 얘는 free블락이라 풋터 필요함
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    size_t newsize    = ALIGN(size)+DSIZE;           // 새 사이즈
    size_t copySize = originsize-WSIZE;//풋터가 없으니까
    ...
    if (newsize <= originsize) {
        ...
        if (!GET_ALLOC(HDRP(NEXT_BLKP(oldptr))) && (newsize <= addSize)) { // 가용 블록이고 사이즈 충분
            ...
            deleteFreeBlock(NEXT_BLKP(oldptr));
            // addFreeBlock(oldptr);// 이러면 헤더 정보가 들어가게된다. alloc인 블록에

            if((addSize- newsize)>=(3*DSIZE)){ // 남는 크기가 2word 이상이면 남는 부분 free하기
                PUT(HDRP(oldptr), PACK(addSize,1)); // 헤더 설정
                // no footer라서 풋터 뺏음
                void* nextptr=NEXT_BLKP(oldptr);
                PUT(HDRP(nextptr),PACK(addSize-newsize,0)); // 남는크기만큼 free block의 헤더 설정
                PUT(FTRP(nextptr),PACK(addSize-newsize,0)); // 남는 크기만큼 free block의 풋터 설정
                SET_PREV_FREE(HDRP(nextptr),0); // 분할된 블록 헤더의 prev_free 비트 세팅
                SET_PREV_FREE(FTRP(nextptr),0); // 분할된 블록 풋터의 prev_free 비트 세팅
                addFreeBlock(nextptr);
                SET_PREV_FREE(HDRP(NEXT_BLKP(nextptr)),1);
            }
            else{ //남는 공간이 없으면 그냥 넣기
                PUT(HDRP(oldptr),PACK(addSize,1));
                // PUT(FTRP(oldptr),PACK(addSize,1));
                SET_PREV_FREE(HDRP(NEXT_BLKP(oldptr)),0); // 다음 블록 헤더의 prev_free 비트 세팅
                // 솔직히 생각해봐, freeblock을 콕 찝어서 선택해서 들어왔는데 그 다음이 free 일 수가 없어 즉시 연결이니까
            }
            ...
            if (newptr == NULL)
                return NULL;
            memcpy(newptr, oldptr, copySize); // memcpy 사용 시, memcpy-param-overlap 발생
            mm_free(oldptr);
            return newptr;
        }
    }
    ...
    //         return NULL;        
    //     memmove(newptr, oldptr, oldSize-WSIZE); //oldptr에서 copysize만큼의 값을 newptr로 복사
...
static void* coalesce(void* bp){
    ...
    size_t prev_alloc = !GET_PREV_FREE(HDRP(bp)); // 지금 블락의 prev_free를 보고 이전 블락이 free인지 아닌지 판단
    size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp))); //오른쪽블록의 헤더로부터 가져온 alloc_bit
    size_t size = GET_SIZE(HDRP(bp)); //지금 내 블록의 사이즈

    if(!prev_alloc&& !next_alloc){// 둘다 free 일 경우
        ...
        deleteFreeBlock(NEXT_BLKP(bp)); // 병합하기전에 리스트에서 삭제 해야함
    ...
    else if(!prev_alloc && next_alloc){ //왼쪽만 free 일 경우
        deleteFreeBlock(PREV_BLKP(bp));
        size+=GET_SIZE(HDRP(PREV_BLKP(bp)));
    ...
    // 왼쪽 오른쪽 둘다 allocated이면 그냥 bp 반환 
    SET_PREV_FREE(HDRP(NEXT_BLKP(bp)),1); // 다음 블록 헤더의 prev_free 비트 세팅
    // 다음 블록은 alloc 일테니 풋터가 없음
    addFreeBlock(bp);
    return bp;
}
...
static void* find_fit(size_t asize){
    void *bp;
    for(bp=fl_head;bp!=NULL;bp=GET_NEXT(bp)){ // 프롤로그부터 시작해서 각 블록별 순회
...
/* 이미 free 처리 끝난 블락의 시작주소 bp를 줘야함 */
static void place(void* bp, size_t asize){ // 이 함수는 블록을 받아서 값 입력을 했는데, 남는 공간이 있을 수도 있으니까 그거 분할하는 과정도 포함되어 있음.
    size_t csize=GET_SIZE(HDRP(bp)); // 내가 할당받은 free 블록의 사이즈 (align 되어 있음)
    deleteFreeBlock(bp); //원래  free 였다가 place해서 더이상 free가 아니게됨
    ...
    if((csize- asize)>=(3*DSIZE)){ // 남는 크기가 3word 이상이면 남는 부분 free하기
        // printf("splited : %p, %lu, %lu\n",bp,csize,asize);
        ...
        SET_PREV_FREE(HDRP(bp),0); // 분할된 free 블록 헤더의 prev_free 비트 세팅
        SET_PREV_FREE(FTRP(bp),0); // 분할된 free 블록 풋터의 prev_free 비트 세팅
        // coalesce(bp); // 이걸 안했나? // 이걸 왜하는데?
        addFreeBlock(bp);
        SET_PREV_FREE(HDRP(NEXT_BLKP(bp)),1); // 분할된 free 블록 다음 블록 헤더의 prev_free 비트 세팅
        //분할된 free블록 다음은 alloc일테니 풋터가 없음
    }
    else{ //남는 공간이 없으면 그냥 넣기
        // printf("just put : %p, %lu, %lu\n",bp,csize,asize);
        PUT(HDRP(bp),PACK(csize,1));
        // PUT(FTRP(bp),PACK(csize,1)); // alloc 블록은 풋터가 필요없음
        SET_PREV_FREE(HDRP(NEXT_BLKP(bp)),0); // 다음 블록 헤더의 prev_free 비트 세팅
        // 솔직히 생각해봐, freeblock을 콕 찝어서 선택해서 들어왔는데 그 다음이 free 일 수가 없어 즉시 연결이니까
    }
}
...
static void addFreeBlock(void* newPtr){
    if(fl_head==NULL){ // free list에 아무것도 없다면
        PUT_PREV(newPtr,NULL); // 시작 노드의 이전도 없고
        PUT_NEXT(newPtr,NULL); // 다음도 없음
        fl_head=newPtr; // newptr을 헤드로 임명
    }
    else{ // free list에 하나라도 노드가 있다면, LIFO니까 맨 앞에 추가
        PUT_PREV(newPtr,NULL); // 맨 앞에 올 노드니까 이전은 없고
        PUT_NEXT(newPtr,fl_head); // 다음 노드가 원래 헤드였던 노드
        PUT_PREV(fl_head,newPtr); // 그리고 원래 헤드였던 노드의 이전이 추가한 블락이고
        fl_head=newPtr;// 추가한 블락을 헤드로 임명
    }
}

static void deleteFreeBlock(void* delPtr){
    if(!fl_head||!delPtr) return; // 삭제할건데 아무 노드도 없으면 그냥 리턴
    if(fl_head==delPtr){ // 헤드 노드를 삭제 할 경우
        fl_head=GET_NEXT(delPtr);// 헤드 다음의 노드가 헤드가 될테니
        if(!fl_head) return; // 리스트가 하나밖에 없는 경우
        else PUT_PREV(fl_head,NULL); // 새로운 헤드의 이전은 없어야 하니 널로 설정
    }
    else{
        PUT_NEXT(GET_PREV(delPtr),GET_NEXT(delPtr)); // 이전노드의 다음이 삭제할 노드의 다음
        if(GET_NEXT(delPtr)) PUT_PREV(GET_NEXT(delPtr),GET_PREV(delPtr)); // 다음 노드의 이전이 삭제할 노드의 이전
    }
}